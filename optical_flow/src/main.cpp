...
int computeFlowVideos(string& inputdir, string& traindir, string& outdir, string& format, int method, int verbosity, int device_id, int bound, int start, int end)
{

	std::vector <std::string> dirlist = listdir(traindir);
	//for(int i=dirlist.size()-1;i>=0;i--)
	//for(int i=0;i<dirlist.size();i++)
	//for(int i=start; i>=0; i--)
	srand(time(NULL));
	int nDir = dirlist.size();
	nDir = nDir < 10? nDir * 0.5 : nDir * 0.1;
	nDir = nDir == 0? 1 : nDir;
	
	for(int i=0; i<nDir; i++)
	{
		int index = rand() % dirlist.size();
		string basevidedir = dirlist[index];
		replace(basevidedir,"\(","\\(");
		replace(basevidedir,"\)","\\)");
...
int main(int argc, char **argv)
{
	...
	if(argc != 4){
		cout<<argv[0]<<" <input_dir> <output_dir> <train_dir>"<<endl;
		exit(1);
	}
	const char* keys =
	{
			...
			"{ id  inputDir   | ../frames/ | path of directory which contain images of folders with images where each folder belog to one video }"
			"{ od  outputDir  | ../optical_flow/ | Name of the directory where flow images will be stroed }"
			...
			"{ ev  end  | 300000 | end video number }"
			"{ ims imgscale  | 1 | imgscale }"
	};

	CommandLineParser cmd(argc, argv, keys);
	string inputBasedir = argv[1]; // inputer diretory where images (folder with images) are present from a video or multiple videos
	string outBaseDir = argv[2];  // save flow image in this directory
	...
	else {computeFlowVideos(inputBasedir, trainDir,outBaseDir, format, method, verbosity, device_id, bound, start, end);}
